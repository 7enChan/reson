...
This project provides a command-line tool to convert EPUB ebooks or Markdown (`.md`, `.markdown`) manuscripts into audiobooks. It now supports the [Microsoft Azure Text-to-Speech API](https://learn.microsoft.com/en-us/azure/cognitive-services/speech-service/rest-text-to-speech), [Google Gemini 2.5 Preview TTS](https://ai.google.dev/gemini-api/docs/tts), [Alibaba Qwen3 TTS](https://help.aliyun.com/zh/model-studio/qwen-tts), [OpenAI Text-to-Speech](https://platform.openai.com/docs/guides/text-to-speech), [EdgeTTS](https://github.com/rany2/edge-tts), and [Piper](https://github.com/rhasspy/piper) providers to generate chapter audio. The output audio files are optimized for use with [Audiobookshelf](https://github.com/advplyr/audiobookshelf).

<!-- *This project was developed with the help of ChatGPT.* -->

## Recent Updates

- 2025-09-25: Added Alibaba Qwen3 TTS provider to CLI and WebUI, including DashScope SDK integration.
...
- For using *Gemini 2.5 Preview TTS*, a Google AI Studio project with a `GOOGLE_API_KEY` (and preview access) is required. The Gemini SDK is currently in preview; review the usage limits before production use.
- For using *Qwen3 TTS*, an Alibaba Cloud DashScope account with a `DASHSCOPE_API_KEY` is required. Install the DashScope Python SDK (`pip install dashscope>=1.24.6`).
- Piper TTS executable and models for *Piper TTS*

## Audiobookshelf Integration

The audiobooks generated by this project are optimized for use with [Audiobookshelf](https://github.com/advplyr/audiobookshelf). Each chapter derived from the source document is converted into a separate MP3 file, with the chapter title extracted and included as metadata.
...
For Markdown manuscripts, top-level (`#`) and second-level (`##`) headings are treated as chapter boundaries. Heading text becomes the chapter title, and the parser removes common Markdown formatting (links, emphasis, lists, etc.) before passing content to the TTS engine.

Please note that these approaches may not work perfectly for every document, especially those with complex or unusual formatting. However, in most cases, they provide a reliable way to extract chapter titles for use in Audiobookshelf.
...
4. Set the following environment variables with your Azure Text-to-Speech API credentials, or your OpenAI API key if you're using OpenAI TTS:
   ...
   export GOOGLE_API_KEY=<your_gemini_api_key> # for Google Gemini TTS (Preview)
   export DASHSCOPE_API_KEY=<your_dashscope_api_key> # for Alibaba Qwen3 TTS
   ```
...
export GOOGLE_API_KEY=<your_gemini_api_key>    # For Gemini 2.5 Preview TTS
export DASHSCOPE_API_KEY=<your_dashscope_api_key> # For Qwen3 TTS
```
...
3. **Configure provider-specific settings**:
   ...
   - **Gemini**: Pick model, voice, output format, speaker map, and temperature (0.0–1.0)
   - **Qwen3**: Configure DashScope API key, model snapshot, voice, language type, streaming toggle, and download timeout
...
To convert an EPUB or Markdown file to an audiobook, run the following command, specifying the TTS provider of your choice with the `--tts` option:
...
```bash
usage: main.py [-h] [--tts {azure,openai,edge,gemini,qwen3,piper}]
               ...
               [--gemini_audio_encoding GEMINI_AUDIO_ENCODING]
               [--gemini_sample_rate GEMINI_SAMPLE_RATE]
               [--gemini_channels GEMINI_CHANNELS]
               [--gemini_speaker_map GEMINI_SPEAKER_MAP]
               [--gemini_temperature GEMINI_TEMPERATURE]
               [--qwen_api_key QWEN_API_KEY]
               [--qwen_language_type {Chinese,English,Spanish,Russian,Italian,French,Korean,Japanese,German,Portuguese}]
               [--qwen_stream] [--qwen_request_timeout QWEN_REQUEST_TIMEOUT]
...
positional arguments:
  input_file            Path to the EPUB or Markdown (.md/.markdown) file
  output_folder         Path to the output folder

options:
  -h, --help            show this help message and exit
      --tts {azure,openai,edge,gemini,qwen3,piper}
                                                         Choose TTS provider (default: azure). azure: Azure
                                                         Cognitive Services, openai: OpenAI TTS API, edge:
                                                         Microsoft Edge voices, gemini: Google Gemini 2.5 Preview
                                                         TTS, qwen3: Alibaba Qwen3 TTS, piper: Piper local/Docker
                                                         voices. When using azure, environment variables MS_TTS_KEY
                                                         and MS_TTS_REGION must be set. When using openai,
                                                         environment variable OPENAI_API_KEY must be set. When using
                                                         gemini, GOOGLE_API_KEY must be provided (or passed via
                                                         --gemini_api_key). When using qwen3, DASHSCOPE_API_KEY must
                                                         be set (or passed via --qwen_api_key).
...
gemini specific:
   --gemini_api_key GEMINI_API_KEY
                                    Google API key for Gemini TTS. Defaults to GOOGLE_API_KEY environment variable if not provided.
   --gemini_audio_encoding GEMINI_AUDIO_ENCODING
                                    Audio encoding of Gemini inline data payload (default: pcm16)
   --gemini_sample_rate GEMINI_SAMPLE_RATE
                                    Sample rate in Hz for Gemini PCM output (default: 24000)
   --gemini_channels GEMINI_CHANNELS
                                    Number of channels in Gemini PCM output (default: 1)
   --gemini_speaker_map GEMINI_SPEAKER_MAP
                                    JSON object mapping speaker labels to Gemini voice names, e.g. '{"Joe": "Kore", "Jane": "Puck"}'
   --gemini_temperature GEMINI_TEMPERATURE
                                    Sampling temperature for Gemini TTS (0.0-1.0). Lower values sound more consistent. Default: 0.2

qwen3 specific:
   --qwen_api_key QWEN_API_KEY
                                    DashScope API key for Qwen3 TTS. Defaults to DASHSCOPE_API_KEY environment variable if not provided.
   --qwen_language_type {Chinese,English,Spanish,Russian,Italian,French,Korean,Japanese,German,Portuguese}
                                    Language type passed to the DashScope API. Align this with your input text or voice selection.
   --qwen_stream         Enable streaming responses. Audio chunks are reassembled locally before exporting.
   --qwen_request_timeout QWEN_REQUEST_TIMEOUT
                                    Timeout (seconds) for downloading audio from the temporary DashScope URL (default: 30).
...
Replace `$MS_TTS_KEY` and `$MS_TTS_REGION` with your Azure Text-to-Speech API credentials. Replace `$OPENAI_API_KEY` with your OpenAI API key. Replace `your_book.epub` with the path to your input file (EPUB or Markdown), and `audiobook_output` with the name of the directory where you want to save the output files.
