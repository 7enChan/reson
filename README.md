...
> Tip: the WebUI now provides input fields for `MS_TTS_KEY`, `MS_TTS_REGION`, and `GOOGLE_API_KEY`, so you can paste the values directly in the interface if you prefer not to export them beforehand.
...
- **TTS Provider Selection**: Easy switching between Azure, Gemini, OpenAI, Edge, and Piper TTS with provider-specific options
...
```bash
usage: main.py [-h] [--tts {azure,openai,edge,gemini,piper}]
...
options:
  -h, --help            show this help message and exit
   --tts {azure,openai,edge,gemini,piper}
                                    ...
                                    TTS, piper: Piper local/Docker voices. When using azure,
                                    environment variables MS_TTS_KEY and MS_TTS_REGION must
                                    be set. When using openai, environment variable
                                    OPENAI_API_KEY must be set. When using gemini,
                                    GOOGLE_API_KEY must be provided (or passed via
                                    --gemini_api_key).
